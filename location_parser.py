...
def extract_and_normalize_location(query: str) -> str | None:
    ...
    match = re.search(r"""
        (?:.*?(?:I'm\s+at\s+|at\s+|on\s+|cross\s+the\s+)?.*?)
        (?P<street1>(?:\d+(?:st|nd|rd|th)?\s+)?[\w\s'-]+?(?:avenue|ave|street|st|road|rd|boulevard|blvd|drive|dr|place|pl|court|ct))
        \s+(?:and|&|@)\s+
        (?P<street2>(?:\d+(?:st|nd|rd|th)?\s+)?[\w\s'-]+?(?:avenue|ave|street|st|road|rd|boulevard|blvd|drive|dr|place|pl|court|ct))
